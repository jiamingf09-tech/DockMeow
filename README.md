...
>
> If macOS says **"DockMeow.app is damaged and can't be opened"** or asks you to move it to Trash, do not delete the app. This is usually caused by macOS quarantine on unsigned/notarization-pending builds. Open **Terminal** and run:
>
> ```bash
> xattr -cr /Applications/DockMeow.app
> ```
>
> Then right-click **DockMeow.app** → **Open** again. If you installed DockMeow somewhere else, replace `/Applications/DockMeow.app` with the actual app path.
...
>
> 如果 macOS 提示 **“DockMeow.app 已损坏，无法打开”** 或要求移到废纸篓，请不要删除软件。这通常是因为未签名/待公证构建被 macOS 加上了隔离标记。打开「终端」执行：
>
> ```bash
> xattr -cr /Applications/DockMeow.app
> ```
>
> 然后再次右键点击 **DockMeow.app** → **打开**。如果 DockMeow 安装在其他位置，请把 `/Applications/DockMeow.app` 替换成实际路径。
