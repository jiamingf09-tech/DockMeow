...
def _build_html() -> str:
    ...
    return (
        ...
        "  for (var i = 1; i < points.length; i++) { _fallbackCtx.lineTo(points[i].x, points[i].y); }"
        "  _fallbackCtx.stroke();"
        ...
        "  var sx = shape.dimensions.w / 2, sy = shape.dimensions.h / 2, sz = shape.dimensions.d / 2;"
        "  var corners = ["
        ...
        "      if (fmt === 'sdf') {_fallbackParseSdf(text, model);} else {_fallbackParsePdb(text, model);}"
